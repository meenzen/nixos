...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    services = {
      ...
      matrix-synapse-next = lib.mkIf cfg.enableWorkers {
      ...
      redis.servers.synapse = lib.mkIf cfg.enableWorkers {
        enable = true;
        user = serviceName;
      ...
      # Monolithic Synapse
      matrix-synapse = lib.mkIf (!cfg.enableWorkers) {
        enable = !cfg.enableWorkers;
        withJemalloc = true;
        enable_metrics = true;
        plugins = plugins;
      ...
      nginx.virtualHosts."${cfg.matrixDomain}" =
        if cfg.enableWorkers
        then {
          enableACME = false;
          ...
          locations = {
            ...
            "/_synapse/admin".proxyPass = "http://[::1]:${toString cfg.port}";
          };
          extraConfig = ''
            add_header X-Robots-Tag "noindex, nofollow, nosnippet, noarchive";
          '';
